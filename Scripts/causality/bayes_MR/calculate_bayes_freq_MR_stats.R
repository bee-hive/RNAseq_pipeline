...
calc_freq_MR = function(exp_trans_cands, exp_trans_cands_perm, temp_cov, beta_xz, trans_candidates) {
	...
	sig_sq = sapply(c(1:length(beta_mr)), function(n) {(res_y[,n] - res_y[,ncol(res_y)]*beta_mr[n]) %*% (res_y[,n] - res_y[,ncol(res_y)]*beta_mr[n]) / (length(res_g) - 3)})
	var_beta = sig_sq * as.numeric((t(res_g) %*% res_g) / (res_y[,ncol(res_y)] %*% res_g)^2)
	...
	sig_sq = sapply(c(1:length(beta_mr)), function(n) {(res_y_perm[,n] - res_y[,ncol(res_y)]*beta_mr[n]) %*% (res_y_perm[,n] - res_y[,ncol(res_y)]*beta_mr[n]) / (length(res_g) - 3)})
	var_beta = sig_sq * as.numeric((t(res_g) %*% res_g) / (res_y_perm[,ncol(res_y_perm)] %*% res_g)^2)
	trans_candidates_MR_stats$MR_Wald_perm = (beta_mr)^2/var_beta

	return(trans_candidates_MR_stats)
}

calc_MR_ABF = function(exp_cis, exp_trans, temp_cov, gene_list, W_MR) {
	...
	# Now solve exp_trans with respect to all covariates, genotype, and exp_cis
...
for (i in c(1:nrow(cis_eqtl_list))) {
	...
	# The set of trios to test for
	trans_candidates = calc_WABF(exp, temp_cov, W, PO, 0.1)
	if (nrow(trans_candidates) == 0) {next}

	# Calculate frequentist MR Wald stats
	exp_trans_cands = t(expression_matrix)[as.logical(inds),c(rownames(trans_candidates), as.character(cis_eqtl_list[i,]$gene))]
	exp_trans_cands = center_colmeans(exp_trans_cands)
	exp_trans_cands_perm = exp_perm[,c(rownames(trans_candidates))]
	...
	trans_candidates_MR_stats = calc_freq_MR(exp_trans_cands, exp_trans_cands_perm, temp_cov, cis_eqtl_list[i,]$beta, trans_candidates)

	# Calculate Bayesian MR-ABF
	exp_cis = t(expression_matrix)[as.logical(inds),as.character(cis_eqtl_list[i,]$gene)]
	MR_ABF_df = calc_MR_ABF(exp_cis, exp, temp_cov, rownames(trans_candidates), W_MR)
	MR_ABF_df_perm = calc_MR_ABF(exp_cis, exp_perm, temp_cov, rownames(trans_candidates), W_MR)

	total_df = cbind(trans_candidates_MR_stats, MR_ABF_df)
	colnames(MR_ABF_df_perm) = c('H00_ABF_perm', 'H01_ABF_perm', 'H10_ABF_perm', 'H11_ABF_perm', 'PPA_perm')
