...
# args = c(1:8)
# args[1] = '/tigress/BEE/amish/analyses/ciseqtl/genomewide/cis_eqtls_150kb_chr22.txt'
# args[2] = '22'
...
amish_eqtl_prefix = '/tigress/BEE/amish/analyses/ciseqtl/genomewide/cis_eqtls_150kb_chr'
expr_dir = '/tigress/BEE/RNAseq/Data/Expression/gtex/hg19/GTEx_phs000424_v6p/normalized/'
expr_name = 'cellsebvtransformedlymphocytes_v6p_consortium_autosomes_normalized.txt'
version = 'v6p'
...
for i in range(22):
  sbatchfile = '/tigress/BEE/RNAseq/Scripts/eqtls/amish_pipeline/batch/cis_eqtls_amish_comparison_' + str(i+1) + '.slurm'
  sbatchhandle=open(sbatchfile, 'w')
  cmd=r"""#!/bin/bash
#SBATCH -J %s           # job name
#SBATCH --mem=32000             # 32 GB requested
#SBATCH -t 24:00:00
#SBATCH -e /tigress/BEE/RNAseq/Scripts/eqtls/amish_pipeline/batch/cis_eqtls_amish_comparison_%s.err
#SBATCH -o /tigress/BEE/RNAseq/Scripts/eqtls/amish_pipeline/batch/cis_eqtls_amish_comparison_%s.out        

Rscript /tigress/BEE/RNAseq/Scripts/eqtls/amish_pipeline/cis_eqtls_amish_comparison.R %s %s %s %s %s %s %s %s
"""%(str(i+1), str(i+1), str(i+1), amish_eqtl_prefix + str(i+1) + '.txt', str(i+1), expr_dir, expr_name, version, genotype_file_prefix + str(i+1) + '.txt', tissue, out_dir)
