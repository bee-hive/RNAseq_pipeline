...
# Only interested in cis-eQTLs - within 150kb
me = MatrixEQTL_wrapper(genotype_matrix, expression_matrix, snp_positions, gene_positions, pvThresh = 0, pvThresh_cis = 1, covariates = covars, cis_dist = 150000)
...
comp_df = cbind(comp_df, amish_cis_eqtls[rownames(comp_df),c('af','beta','se','l_remle','l_mle','p_wald','p_lrt')])
colnames(comp_df)[c(13:15)] = c('amish_af', 'amish_beta', 'amish_se')

write.table(comp_df, quote = F, sep = '\t', row.names = F, file = paste0(out_dir, tissue_name, '/gtex_amish_comparison_ciseqtls_chr_', chrom, '.txt'))
